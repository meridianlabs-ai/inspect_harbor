...
The `>=1.0` floor matters: 1.0 is the first release that does not depend on
the `harbor` package, so it installs alongside current `openai` releases and
uses the organization-qualified task names these docs describe. Older releases
pin dependencies that conflict with Inspect's and are no longer supported.
