...
REGISTRY_MD_FILE = Path(__file__).parent.parent / "REGISTRY.md"
...
REGISTRY_MD_TEMPLATE = """# Inspect Harbor Registry
...
def generate_registry_md_content(registry: list) -> str:
    """Generate the content of REGISTRY.md from the registry."""
    table_rows = []
    seen_versioned_datasets = set()
    ...
    return REGISTRY_MD_TEMPLATE.format(
        registry_url=REGISTRY_URL,
        table_rows="\n".join(table_rows),
    )
...
def main():
    ...
    registry_md_content = generate_registry_md_content(registry)
    REGISTRY_MD_FILE.parent.mkdir(parents=True, exist_ok=True)
    REGISTRY_MD_FILE.write_text(registry_md_content)
    print(f"✓ Generated: {REGISTRY_MD_FILE}")
    print(f"  Size: {len(registry_md_content)} bytes")
    print(
        f"  Datasets: {len([line for line in registry_md_content.split('\n') if line.startswith('|') and not line.startswith('|---')])}"
    )
