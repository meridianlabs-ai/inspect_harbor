...
async def test_yields_transcripts_from_fixtures_dir(fixtures_dir: Path) -> None:
    """All committed ATIF fixtures yield (covers v1.5 and v1.7)."""
    count = 0
...
async def test_context_management_step_emits_compaction_event(
    ...
    assert event.tokens_after == 30000
...
async def test_yields_subagent_spans_for_real_terminus_2_fixture(
    ...
    Exercises the subagent code path end-to-end through `atif()` on a real
    Harbor run. Subagent files aren't committed, so each ref resolves to an
    empty span shell — but `SpanBeginEvent`/`SpanEndEvent` are still emitted,
    proving v1.7 parses and the subagent dispatch runs through the full
    source pipeline.
    """
    trajectory_file = (
        fixtures_dir
