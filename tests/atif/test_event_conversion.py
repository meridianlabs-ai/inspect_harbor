...
from inspect_harbor._atif.transcripts import _create_subagent_span_events
...
class TestModelEventConversion:
    ...
    def test_llm_call_count_zero(self) -> None:
        """`llm_call_count=0` (deterministic dispatch) → returns None.

        The harbor model forbids `metrics` when `llm_call_count=0`, so this
        branch is hit when a step is missing metrics for that reason. Set
        metrics on the step via `model_construct` to bypass validation and
        exercise the gating directly.
        """
        step = Step.model_construct(
            step_id=1,
            source="agent",
            message="dispatched",
            metrics=Metrics(prompt_tokens=0, completion_tokens=0),
            llm_call_count=0,
        )
        msg = ChatMessageAssistant(content="dispatched")
        assert to_model_event(step, prior_messages=[], new_messages=[msg]) is None
...
class TestCompactionEventConversion:
    ...
    def test_extracts_tokens(self) -> None:
        """Step with `extra.context_management` → CompactionEvent with tokens."""
        step = Step(
            step_id=1,
            source="system",
            message="(compacted)",
        ...
        assert result.tokens_before == 150000
        assert result.tokens_after == 30000

    def test_clamps_invalid_type_to_summary(self) -> None:
        """Unknown `type` value → falls back to 'summary'."""
        step = Step(
            step_id=1,
            source="system",
            message="x",
            extra={"context_management": {"type": "not-a-real-type"}},
        )
        assert to_compaction_event(step).type == "summary"

    def test_handles_missing_extra(self) -> None:
        """No `context_management` in `extra` → default `type='summary'`, no tokens."""
...
class TestExtractContentBlocks:
    ...
    def test_multipart_text(self) -> None:
        ...
        assert [b.text for b in result] == ["first", "second"]  # type: ignore[attr-defined]
