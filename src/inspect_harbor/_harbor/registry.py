...
async def load_registry(
    ...
    Like Harbor, the URL is fetched on every load; nothing is cached on disk.
    ...
    return parse_registry(await _fetch_text(url, client), url)
...
async def resolve_registry_dataset(
    ...
    datasets = await load_registry(url=url, path=path, client=client)
